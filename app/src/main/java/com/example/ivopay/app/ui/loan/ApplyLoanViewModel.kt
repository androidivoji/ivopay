...
class ApplyLoanViewModel(private val context: Context) : ViewModel() {
    ...
    var riplayPoints = mutableStateListOf<RiplayPoint>()
    ...
    fun toggleRiplayPoint(index: Int, checked: Boolean) {
        riplayPoints[index] = riplayPoints[index].copy(checked = checked)
        // Jika ada satu saja yang tidak dicentang, maka status persetujuan utama harus batal
        if (!checked) {
            isRiplayAgreed = false
        }
    }
    ...
    fun onNextClick(onSign: () -> Unit, onSuccess: (String) -> Unit, onError: (String) -> Unit) {
        if (!isRiplayAgreed) {
            onError("Harap setujui Ringkasan Informasi Produk dan Layanan (RIPLAY)")
            return
        }
    ...
    fun submitApply() {
        ...
        viewModelScope.launch {
            try {
                ...
                builder.addFormDataPart("agreementConfirmed", "true")
        ...
        viewModelScope.launch {
            try {
                ...
                val requestBody = MultipartBody.Builder()
                    ...
                    .addFormDataPart("agreementConfirmed", "true")
                    .addFormDataPart(
                        "aig", 
                        "mock_signature.png", 
